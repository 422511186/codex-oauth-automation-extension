...
test('auto-run controller still skips add-phone failures to the next round when auto retry is disabled', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  assert.equal(events.runCalls, 2, 'add-phone failure should still skip to the next round even when auto retry is disabled');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'retrying'), false, 'add-phone failure should not enter same-round retrying');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), false, 'add-phone failure should not stop the whole auto-run when later rounds remain');
  assert.equal(events.accountRecords.length, 1, 'fatal add-phone should still persist a failed round record');
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /add-phone/);
  assert.ok(events.logs.some(({ message }) => /继续下一轮/.test(message)));
...
test('auto-run controller skips 163 helper login failures to the next round instead of retrying the same round', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    hasSavedProgress: () => false,
    isAddPhoneAuthFailure: () => false,
    isMail163LoginAuthFailure: (error) => /LOGIN Login error or password error/i.test(error?.message || String(error || '')),
    isRestartCurrentAttemptError: () => false,
    isStopError: (error) => (error?.message || String(error || '')) === '流程已被用户停止。',
    launchAutoRunTimerPlan: async () => false,
    normalizeAutoRunFallbackThreadIntervalMinutes: (value) => Math.max(0, Math.floor(Number(value) || 0)),
  ...
  assert.equal(events.runCalls, 2, '163 helper login failure should skip the current round and continue with the next round');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'retrying'), false, '163 helper login failure should not enter same-round retrying');
  assert.equal(events.accountRecords.length, 1, '163 helper login failure should still persist a failed round record');
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /LOGIN Login error or password error/);
  assert.ok(events.logs.some(({ message }) => /继续下一轮/.test(message)));
  assert.equal(runtime.state.autoRunActive, false);
  assert.equal(runtime.state.autoRunSessionId, 0);
});

test('auto-run controller still skips 163 helper login failures to the next round when auto retry is disabled', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  assert.equal(events.runCalls, 2, '163 helper login failure should still skip to the next round even when auto retry is disabled');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'retrying'), false, '163 helper login failure should not enter same-round retrying');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), false, '163 helper login failure should not stop the whole auto-run when later rounds remain');
  assert.equal(events.accountRecords.length, 1, '163 helper login failure should still persist a failed round record');
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /LOGIN Login error or password error/);
  assert.ok(events.logs.some(({ message }) => /继续下一轮/.test(message)));
...
test('auto-run controller clears OpenAI cookies before the next fresh round after an error', async () => {
  const events = {
    cleanupCalls: [],
    logs: [],
    broadcasts: [],
  ...
  const controller = api.createAutoRunController({
    ...
    runOpenAiCookieCleanup: async (options = {}) => {
      events.cleanupCalls.push({
        runCalls: events.runCalls,
        options,
      });
    },
    runAutoSequenceFromStep: async () => {
  ...
  assert.equal(events.runCalls, 2, 'controller should continue into the next round after the first failure');
  assert.deepStrictEqual(events.cleanupCalls, [
    {
      runCalls: 1,
      options: {
        logPrefix: '自动运行：第 2/2 轮第 1 次尝试重开前',
        waitMs: 0,
...
test('auto-run controller retries the current account when the active tab id becomes invalid', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    isRetryableAutoRunTabGoneError: (error) => /No tab with id:\s*\d+/i.test(error?.message || String(error || '')),
    isRestartCurrentAttemptError: () => false,
    isSignupUserAlreadyExistsFailure: () => false,
    isStopError: (error) => (error?.message || String(error || '')) === '流程已被用户停止。',
    launchAutoRunTimerPlan: async () => false,
    normalizeAutoRunFallbackThreadIntervalMinutes: (value) => Math.max(0, Math.floor(Number(value) || 0)),
    patchMail163Account: async (_accountId, updates) => {
      events.patchCalls.push(updates);
      return updates;
    },
    persistAutoRunTimerPlan: async () => ({}),
    resetState: async () => {
    ...
    runAutoSequenceFromStep: async () => {
      events.runCalls += 1;
      if (events.runCalls === 1) {
        throw new Error('No tab with id: 650894013');
      }
    },
    runtime,
    setState: async (updates = {}) => {
  ...
  assert.equal(events.runCalls, 2, 'missing-tab error should retry the same current account');
  assert.equal(events.patchCalls.length, 0, 'current mail163 account should not be marked failed before retry');
  assert.equal(events.accountRecords.length, 0, 'successful retry should not persist a failed record');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'retrying'), true, 'missing-tab error should enter retrying phase');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), false, 'missing-tab retry should not stop auto-run');
  assert.ok(events.logs.some(({ message }) => /失效标签页/.test(message)));
});
