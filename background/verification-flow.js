  ...
  function createVerificationFlowHelpers(deps = {}) {
    ...
    async function submitVerificationCode(step, code, options = {}) {
      ...
      try {
        ...
        const isRetryableTransportError = isRetryableVerificationTransportError(err)
          || /back\/forward cache|message channel is closed|Receiving end does not exist|port closed before a response was received|A listener indicated an asynchronous response|did not respond in \d+s/i.test(errorMessage);
